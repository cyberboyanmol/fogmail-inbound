...
class Mailin extends events.EventEmitter {
  ...
  public async start(options: object, callback: (err?) => void) {
      ...
      return retrieveRawEmail(connection)
        ...
        .then(function ([
          ...
          return Promise.all([
            ...
            spamScore,
            parsedEmail,
            detectLanguage(connection, parsedEmail),
    ...
    function computeSpamScore(connection, rawEmail) {
      ...
      return mailUtilities
        .computeSpamScoreAsync(rawEmail)
        .catch(function (err) {
          logger.error(
            connection.id +
    ...
    function onData(stream, session, onDataCallback) {
      try {
        ...
        logger.error(
          'Exception occurred while performing onData callback',
          LOG_CONTEXT,
        );
        logger.error(error);
      }
    ...
    server.listen(configuration.port, configuration.host, function () {
      logger.info(
        'Mailin Smtp server listening on port ' + configuration.port,
        LOG_CONTEXT,
      );
    });
